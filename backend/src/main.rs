...
use models::{
    ...
    CreateRepository, CreateTask, CreateUser, GitHubToken, MessageWithRun, OnboardingStatus,
    RepositoryTS, RepositoryWithTasks, Run, RunWithMeta, SetDefaultRepoRequest, Task, TaskDetails,
    TaskLog, TaskLogsPaginated, TaskWithRun, UpdateApiKeysRequest, User, UserWithDefaultRepo,
};
...
async fn handle_task_success(
    ...
    // ——— NEW: handle subsequent runs first (no sandbox/branch required) ———
    let is_subsequent_run = task.github_pr_url.is_some();
    if is_subsequent_run {
        tracing::info!(
            "Task {} subsequent run: posting PR comment instead of creating PR",
            task_id
        );

        // get the current run's final message for the comment
        let run = match app_state.database.get_latest_run_id_for_task(task_id).await {
            Ok(Some(run_id)) => app_state.database.get_run_by_id(run_id).await?,
            _ => None,
        };

        let final_message = run.and_then(|r| r.final_message_md).unwrap_or_default();

        if final_message.trim().is_empty() {
            tracing::error!(
                "No final message available for subsequent run of task {}",
                task_id
            );
            if let Ok(Some(run_id)) = app_state.database.get_latest_run_id_for_task(task_id).await {
                let _ = app_state.database.update_run_status(run_id, "failed").await;
        ...
        let (owner, repo, pr_number) = match GitHubPRClient::parse_pr_url(pr_url) {
            ...
            Err(e) => {
                tracing::error!(
                    "Failed to parse existing PR URL for task {}: {}",
                    task_id,
                    e
                );
                if let Ok(Some(run_id)) =
                    app_state.database.get_latest_run_id_for_task(task_id).await
                {
                    let _ = app_state.database.update_run_status(run_id, "failed").await;
                }
                return Ok(());
        ...
        let comment_body = format!("{}\n*Generated by Swarm AI agent*", final_message);

        match pr_client
            .add_pr_comment(&owner, &repo, pr_number, &comment_body)
            .await
        {
            Ok(comment_url) => {
                tracing::info!(
            ...
            Err(e) => {
                tracing::error!("Failed to add comment to PR for task {}: {}", task_id, e);
                if let Ok(Some(run_id)) =
                    app_state.database.get_latest_run_id_for_task(task_id).await
                {
                    let _ = app_state.database.update_run_status(run_id, "failed").await;
        ...
        // Mark run done; we didn't open a PR in this run.
        if let Ok(Some(run_id)) = app_state.database.get_latest_run_id_for_task(task_id).await {
            if let Err(e) = app_state.database.update_run_status(run_id, "done").await {
                tracing::error!("Error updating run {} status to done: {}", run_id, e);
            }
        }
        tracing::info!("✓ Task {} subsequent run completed successfully", task_id);
        return Ok(());
    }
    // ——— end subsequent run fast-path ———

    // FIRST run path (create/update PR). Now it's safe to use sandbox/branch.
    let run = match app_state.database.get_latest_run_id_for_task(task_id).await {
    ...
    tracing::info!(
        "Using pre-pushed branch {} for task {} (no backend push)",
        branch,
        task_id
    );
    ...
    tracing::info!(
        "✓ PR created for task {}, sandbox {} will be cleaned up by idle timeout",
        task_id,
        sandbox_id
    );
    Ok(())
...
async fn main() -> AppResult<()> {
    ...
    let app = Router::new()
        ...
        .route(
            ...
            get(get_onboarding_status_endpoint),
        )
...
async fn set_default_repo(
    ...
    match app_state
        .database
        .set_default_repository(user.id, Some(payload.repository_id))
        .await
    {
    ...
    match get_onboarding_status(&app_state.database.pool, user.id).await {
        Ok(status) => {
            if status.has_anthropic && status.has_openai && !status.has_default_repo {
                ...
                if let Err(_) = app_state
                    .database
                    .update_onboarding_step(user.id, Some("default-repo".to_string()))
                    .await
                {
...
async fn set_default_repo_onboarding(
    ...
    match app_state
        .database
        .ensure_repo_owner(payload.repository_id, user.id)
        .await
    {
        Ok(_) => {
                ...
                .await
            {
                Ok(_) => {
                    ...
                    match get_onboarding_status(&app_state.database.pool, user.id).await {
                        Ok(status) => {
                            if (status.has_anthropic || status.has_openai)
                                && status.has_default_repo
                            {
                                // Has keys and repo, complete onboarding
                                if let Err(_) =
                                    app_state.database.complete_onboarding(user.id).await
                                {
                                    tracing::warn!(
                                        "Failed to complete onboarding for user {}",
                                        user.id
                                    );
                                }
                            }
...
async fn get_task_details(
    ...
    app_state
        .database
        .ensure_task_owner(task_id, db_user.id)
        .await
