...
pub async fn run_full_task_pipeline(app_state: AppState, task: Task) -> Result<()> {
    ...
    let repository = match app_state.database.get_repository_by_id(task.repository_id, task.user_id).await {
        Ok(Some(repo)) => repo,
        Ok(None) => {
            tracing::error!("Repository {} not found for task {}", task.repository_id, task.id);
            let _ = app_state.database.update_task_status(task.id, "failed", None).await;
            return Err(anyhow::anyhow!("Repository not found"));
        }
        Err(e) => {
            tracing::error!("Database error getting repository {}: {}", task.repository_id, e);
            let _ = app_state.database.update_task_status(task.id, "failed", None).await;
            return Err(anyhow::anyhow!("Database error: {}", e));
        }
    };
    ...
    let github_token = match app_state.database.get_github_token(user.id).await {
        ...
        Ok(None) => {
            tracing::error!("No GitHub token available for user {} in task {}", user.id, task.id);
            let _ = app_state.database.update_task_status(task.id, "failed", None).await;
    ...
    let anthropic_api_key = match user.anthropic_api_key {
        Some(key) => key,
        None => {
            tracing::error!("No Anthropic API key available for user {} in task {}", user.id, task.id);
            let _ = app_state.database.update_task_status(task.id, "failed", None).await;
    ...
    let sandbox_info = match app_state.sandbox.start_sandbox(
        task.id,
        &repo_url,
        &github_token,
        prompt,
    ...
    match app_state.database.update_task_sandbox(
        task.id,
        &sandbox_info.id,
        ...
        Ok(_) => {
            tracing::info!("Updated task {} with sandbox {} info", task.id, sandbox_info.id);
        }
        Err(e) => {
