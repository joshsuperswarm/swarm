...
impl PrStatusPoller {
    ...
    pub async fn start_polling(&self) {
        tracing::info!(
            "Starting PR status poller (interval: {}m)",
    ...
    async fn poll_pr_statuses(&self) -> Result<()> {
        ...
        for task in tasks {
            let Some(pr_url) = &task.github_pr_url else {
                continue;
            };
            ...
            let (owner, repo, number) = match GitHubPRClient::parse_pr_url(pr_url) {
                ...
                Err(e) => {
                    errors += 1;
                    tracing::warn!("Bad PR URL on task {}: {} ({})", task.id, e, pr_url);
                    continue;
                }
            };
            ...
            let client = match GitHubPRClient::new(&token) {
                ...
                Err(e) => {
                    errors += 1;
                    tracing::error!("Make client failed for task {}: {}", task.id, e);
                    continue;
                }
            };
            ...
            match client.is_merged(&owner, &repo, number).await {
                Ok(true) => {
                    if let Err(e) = self.db.update_task_to_pr_merged(task.id).await {
                        errors += 1;
                        tracing::error!("Mark merged failed for task {}: {}", task.id, e);
                    } else {
                        merged += 1;
...
mod tests {
    ...
    async fn test_get_tasks_needing_pr_polling() {
        ...
        // Clean up test data
        let _ = sqlx::query!("DELETE FROM tasks WHERE title LIKE 'Test PR Polling%'")
            .execute(&db.pool)
            .await;
        let _ = sqlx::query!("DELETE FROM users WHERE clerk_user_id = 'test_pr_polling'")
            .execute(&db.pool)
            .await;
        ...
        let _ = sqlx::query!("DELETE FROM tasks WHERE id = $1", task.id)
            .execute(&db.pool)
            .await;
        let _ = sqlx::query!("DELETE FROM repositories WHERE id = $1", repo.id)
            .execute(&db.pool)
            .await;
