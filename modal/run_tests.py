...
def test_create_sandbox():
    ...
    response = requests.post(f"{BASE_URL}/sandboxes", json={
...
def test_exec_command(sandbox_id):
    ...
    response = requests.post(f"{BASE_URL}/sandboxes/{sandbox_id}/exec", json={
...
def test_poll_process(sandbox_id, proc_id):
    ...
    for i in range(10):  # Poll for up to 10 seconds
        response = requests.get(f"{BASE_URL}/sandboxes/{sandbox_id}/procs/{proc_id}/exit_code")
...
def test_get_logs(sandbox_id, proc_id):
    ...
    if response.status_code == 200:
        data = response.json()
        print(f"✓ Retrieved logs - stdout: '{data['stdout']}', stderr: '{data['stderr']}'")
        return True
    else:
